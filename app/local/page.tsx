...
import { getBestMoveLocal } from '@/lib/client-engine';
...
export default function LocalEngine() {
  ...
  const updateStatusUI = useCallback((systemMove: any = null) => {
    ...
    if (chess.turn() === oppColorRef.current) {
      ...
      if (systemMove) {
        ...
        setExplanationHtml(generateExplanation(systemMove));
  ...
  const startEngineCalculation = useCallback(() => {
    setStatusUserTurn(false);
    ...
    setTimeout(() => {
      ...
      const bestMove = getBestMoveLocal(chess, myColorRef.current, oppColorRef.current);
      if (!bestMove) {
        handleGameOver();
        return;
      }
  ...
  return (
    <div className="main-wrapper">
      <div className="left-panel">
        <h2>♟️ Chess Assistant <span style={{ color: '#e11d48' }}>LOCAL</span></h2>
